...
model, vocab, tokenizer = get_pretrained(ctxs, 'gpt2-345m-en-cased')
...
for index,i in enumerate(dfs['sentence']):
    if not pd.isnull(dfs.loc[index,'v']):
        ...
        findVMpO=False
        findVOpM=False
        sentencePatternList=['originalPattern','MVOList','MVpOList','OVMList','OVpMList','VOpMList','VpOpMList','VMpOList','VpMpOList','MList','OBVList','BVOList','OVBList','VOBList','MVpOPassiveList','MpOVPassiveList','OVpMPassiveList']
        sentencePatternNameList=['originalNominalization','MVO','MVpO','OVM','OVpM','VOpM','VpOpM','VMpO','VpMpO','M','OBV','BVO','OVB','VOB','MVpOPassive','MpOVPassive','OVpMPassive']
        ...
        for name,pattern in zip(sentencePatternNameList,sentencePatternList):
            if not pd.isnull(dfs.loc[index,pattern]):
                ...
                if tempResult!=[]:
                    dfs.loc[index,name]=pattern[tempResult.index(max(tempResult))]
sentencePattern=['originalNominalization','MVO','MVpO','OVM','OVpM','VOpM','VpOpM','VMpO','VpMpO','M','OBV','BVO','OVB','VOB','MVpOPassive','MpOVPassive','OVpMPassive']
for index,i in enumerate(dfs['sentence']):
    if not pd.isnull(dfs.loc[index,'v']):
        sentenceList=[]
        scoreDict={}
...
dfs.to_excel("FinalDatasetAutoGPTLargeBaseline.xlsx")